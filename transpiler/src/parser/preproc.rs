...
pub enum Directive {
    ...
    DefineObject { name: String, value: String },
...
pub fn parse_directive(raw: &str) -> Directive {
    ...
    let after_hash = trimmed.trim_start().strip_prefix('#').unwrap_or(trimmed).trim_start();
    let (kw, rest) = split_keyword(after_hash);
    let rest = rest.trim();
    match kw {
        "include" => parse_include(rest, trimmed),
        "define" => parse_define(rest),
        "undef" => Directive::Undef { name: rest.to_string() },
        "ifdef" => Directive::IfDef { name: rest.to_string(), negate: false },
        "ifndef" => Directive::IfDef { name: rest.to_string(), negate: true },
        "if" => Directive::If { expr: rest.to_string() },
        "elif" => Directive::Elif { expr: rest.to_string() },
        "else" => Directive::Else,
...
fn parse_include(rest: &str, whole: &str) -> Directive {
    if let Some(p) = rest.strip_prefix('<').and_then(|s| s.rsplit_once('>').map(|(p, _)| p)) {
        Directive::Include { path: p.to_string(), angled: true }
    } else if let Some(p) = rest.strip_prefix('"').and_then(|s| s.rsplit_once('"').map(|(p, _)| p)) {
        Directive::Include { path: p.to_string(), angled: false }
    } else {
...
fn parse_define(rest: &str) -> Directive {
    ...
    if let Some(paren_rest) = after_name.strip_prefix('(') {
        if let Some(close) = paren_rest.find(')') {
            ...
            return Directive::DefineFunction { name: name.to_string(), params, body };
        }
    }
    Directive::DefineObject { name: name.to_string(), value: after_name.trim().to_string() }
}
